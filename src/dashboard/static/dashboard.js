...
async function loadAll() {
    const endpoints = [
        ['langPop', '/api/language-popularity'],
        ['topRepos', '/api/top-repos'],
        ['topics', '/api/topic-trends'],
        ['growth', '/api/yoy-growth'],
        ['starDist', '/api/star-distribution'],
        ['activity', '/api/activity-index'],
...
function initUserSearch() {
    ...
    async function doSearch() {
        ...
        try {
            const resp = await fetch(`/api/github-user/${encodeURIComponent(query)}`);
